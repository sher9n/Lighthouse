...
class Vote{
    ...
    public static function getUserVotes($adr,$com_id,$type=null) {
        $results = array();
        if(!is_null($type))
            $votes = Vote::find("SELECT * FROM votes WHERE comunity_id=$com_id  AND type='$type' AND wallet_adr='$adr'");
        else
            $votes = Vote::find("SELECT * FROM votes WHERE comunity_id=$com_id AND wallet_adr='$adr'");

        if($votes != false){
            foreach ($votes as $vote) {
                $results[$vote['type_id']] = $vote['vote'];
            }
        }
